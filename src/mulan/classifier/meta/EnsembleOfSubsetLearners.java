...
/**
 * A class for gathering several different SubsetLearners into a composite ensemble model. <br>
 * <br>
 * The label set partitions for participation in ensemble are selected using their dependence weight
 * from the large number of randomly generated possible partitions. The type of the learned
 * dependencies is determined by the {@link mulan.data.LabelPairsDependenceIdentifier} supplied to
 * the class constructor. Two strategies for selecting ensemble partitions exists: (1) to select the
 * highly weighted ones and (2) to select most different from the highly weighted ones. The strategy
 * to be used is determined by the {@link #selectDiverseModels} parameter which is 'true' by
 * default.
 * 
...
public class EnsembleOfSubsetLearners extends MultiLabelMetaLearner {
	...
	/**
	 * Number of highly weighted partitions used for selecting the 'enough' different among them.
	 * Used when {@link #selectDiverseModels} is true.
	 */
	private static int numOfPartitionsForDiversity = 100;
	/**
	 * Parameter used to dynamically define the threshold of 'enough' different partition. Used when
	 ...
	 * 
	 * @throws Exception if was not initialized successfully
	 */
	public EnsembleOfSubsetLearners() throws Exception {
		rnd = new Random(seed);
	}

	/**
	 * Initialize EnsembleOfSubset with multilabel and single label learners, a method for labels
	 * dependence identification and number of models to constitute the ensemble.
	 * 
	 ...
	 * @param aDependenceIdentifier the method for label pairs dependence identification
	...
	/**
	 * Builds an ensemble of Subset models. Label set partitions for ensemble are selected based on
	 * the set {@link #dependenceIdentifier} and value of the {@link #selectDiverseModels}
	 * parameter.
	 * 
	...
	/**
	 * Makes classification prediction using constructed ensemble of Subset models. For each label
	 * models' votes are summarized, and confidence is computed as average of positive predictions.
	 * 
	 * @param instance the data instance to predict on
	 * @return the {@link mulan.classifier.MultiLabelOutput} classification prediction for the
	 * instance.
	 * @throws Exception
	 */
	...
	/**
	 * Creates the specified number of randomly generated possible label set partitions consisting
	 * of the specified number of labels..
	 * 
	 ...
	 * @param normalized indicates if to apply normalization using chiCritical value
	 ...
	 * 
	 * @param n indicates the length of a permutation: each permutation will contain 'n' numbers
	...
	/**
	 * Converts a list of arrays (i.e label subsets) into two-dimensional array representing a label
	 * set partitioning.
	 * 
	...
	/**
	 * Computes 'dependence' score of each partition and store it in a list of weighted subsets.
	 * 
	 ...
	 * 
	 * @param otherLabels the set of labels which pair-weight with the specified label should be
	 * summarized
	 * @param label the label which pair-weight with other labels should be summarized
	 * @param matrix the matrix containing dependence score for each labels pair
	 * @return sum of the weights of pairs of the label with each one of the other labels
	 */
	...
	/**
	 * Filtering out identical label set partitions. An assumption is that partitions with the same
	 * weight are identical.
	 * 
	 * @param orderedList the list of weighted label subsets ordered by subset weight
	 ...
	 * 
	 * @param orderedList the list of weighted label subsets ordered by subset weight in descending
	 * order
	...
	/**
	 * Represents a partition as a matrix: if two labels are in the same group the relatesd entry of
	 * the matrix is 1, otherwise 0.
	 * 
	...
	/**
	 * Selects most different from the highly weighted partitions. This method is used when the
	 * {@link #selectDiverseModels} parameter is 'true'.
	 * 
	 * @param sets the list of weighted label subsets ordered by subset weight in descending order
	...
	/**
	 * Computes the minimal distance from each labels set partition to the "selected" partitions. If
	 * a partition is within the "selected" it's minimal distance is 0.
	 * 
	 * @param allSets the list of all label set partitions
	 * @param selected the list of selected label set partitions
	 * @return an array containing the minimal distance from each label set partition to the
	 * "selected" partitions
	 */
	...
	/**
	 * A class for handling partitions and their dependence weights. The natural order of instances
	 * of this class is according to the natural order of the partition dependence weight.
	 */
	private class LabelSubsetsWeight implements Comparable, Cloneable {
		...
		/**
		 * Initialize a LabelSubsetsWeight object using labels set partition and its weight.
		 * 
	...
	/**
	 * A class for handling partitions and their distance values using partition order identifiers.
	 * The natural order of instances of this class is according to the natural order of partition
	 * distance values.
	 */
	private class SubsetsDistance implements Comparable {
		...
		/**
		 * Initialize a SubsetsDistance object using the partition identifier and its distance
		 * value.
		 * 
	...
	/**
	 * A comparator class for comparing SubsetsDistance objects by partition identifier values.
	 */
