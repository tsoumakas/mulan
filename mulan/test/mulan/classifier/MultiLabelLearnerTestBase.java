...
public abstract class MultiLabelLearnerTestBase {
    ...
    public void testBuild_WithDifferentOrder() throws Exception {
        ...
        /*
        Note: The re-ordering retains the relative order of labels. The reason
        for this is, because we are comparing the outputs of the learners,
        ...
        */
        ...
        Instances transformedTrainData = Filter.useFilter(originalTrainData, reorder);// apply filter

        Instances transformedTestData = Filter.useFilter(originalTestData, reorder);// apply filter
        ...
        for (int i = 0; i < testDataSet.getDataSet().numInstances(); i++) {
            MultiLabelOutput mlo1 = learner1.makePrediction(testDataSet.getDataSet().instance(i));
            MultiLabelOutput mlo2 = learner2.makePrediction(transformedTestDataSet.getDataSet().instance(i));
            Assert.assertEquals(mlo1, mlo2);
        }

        /*
        Note: it may be more meaningful to use the following type of test for
        arbitrary reorderings of labels, but the problem lies in the calculation
        of ranking measures, in the typical case where the ranking is obtained
        from scores, solving the ties through weka.core.StableSort. Different
        orderings of the labels will produce small variations in ranking
        measures

        Evaluator eval = new Evaluator();
        Evaluation results;

        results = eval.evaluate(learner1,testDataSet);
        String OriginalResults = results.toString();
        results = eval.evaluate(learner2, transformedTestDataSet); String
