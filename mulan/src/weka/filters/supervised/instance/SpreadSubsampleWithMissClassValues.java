 ...
 * 
 * !!We make the SpreadSubsampleWithMissClassValues based on the SpreadSubsample by adding the 
 * ability to dealing with instances with missing class values.
 * 
 * <p/>
 * <!-- globalinfo-end -->
 * 
 ...
 * @version $Revision$
 **/
public class SpreadSubsampleWithMissClassValues extends Filter implements SupervisedFilter,
  OptionHandler {
  ...
  /**
   *True if instances with missing class value will be deleted from the output instances,
   *False if instances with missing class value will be added in the output instances without any sampling 
   */
 ...
 */
  public boolean isDeleteInstancesWithMissingClass() {
	  return isDeleteInstancesWithMissingClass;
  }

  /**
   * @param isDeleteInstancesWithMissingClass the isDeleteInstancesWithMissingClass to set
   */
  public void setDeleteInstancesWithMissingClass(boolean isDeleteInstancesWithMissingClass) {
	  this.isDeleteInstancesWithMissingClass = isDeleteInstancesWithMissingClass;
  }
...
/**
  ...
  public Capabilities getCapabilities() {
    ...
    result.enable(Capability.MISSING_CLASS_VALUES);//!!!!Add This!!!!
  ...
  private void createSubsample() {
    ...
    getInputFormat().sort(classI);  //Instances with missing values for the attribute are placed at the end of the dataset
        ...
        do {
          ...
          if (t.get("" + index) == null) {
            ...
            if (index >= 0) {
              ...
              push(newInst); // No need to copy instance
            }
          }
        } while (!ok);
      }
    ...
    if(!isDeleteInstancesWithMissingClass){
    	for (int i = 0; i < getInputFormat().numInstances(); i++) {
    	      Instance current = getInputFormat().instance(i);
    	      if(current.classIsMissing()){
    	    	  Instance newInst = (Instance) current.copy();
    	    	  push(newInst); 
    	      }
    	}
    }
  ...
  private int[] getClassIndices() {
    ...
    int firstMissingClassI=getInputFormat().numInstances();  //if no missing class value, the firstMissingClassI equls getInputFormat().numInstances()
    for (int i = 0; i < getInputFormat().numInstances(); i++) {
      Instance current = getInputFormat().instance(i);
      
      //System.out.println(i+" "+current.toString(current.classIndex()));
      
      if (current.classIsMissing()) {
        for (int j = currentClass + 1; j < classIndices.length; j++) {
          classIndices[j] = i;
        }
        firstMissingClassI=i;
        break;
    ...
    if (currentClass <= getInputFormat().numClasses()) {
      for (int j = currentClass + 1; j < classIndices.length; j++) {
        classIndices[j] = firstMissingClassI;//getInputFormat().numInstances();
      }
    }
    return classIndices;
  }
