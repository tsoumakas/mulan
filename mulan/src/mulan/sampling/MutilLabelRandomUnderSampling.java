...
public class MutilLabelRandomUnderSampling extends MultiLabelSampling{
	...
	public MultiLabelInstances build(MultiLabelInstances mlDataset) throws Exception{
		...
		double MeanIR=is.getMeanIRLb();
		int c1[]=is.getC1();
		
		for(int i=0,j=0;i<labelNum;i++){
			if(IRLbls[i]<MeanIR){
				isMajLabel[i]=true;
			}
		}
		...
		for(int i=0;i<ins.numInstances();i++){
			...
			for(int j=0;j<labelNum;j++){
				...
				*/
				if(d.attribute(labelIndices[j]).value((int)d.value(labelIndices[j])).equals("1")) 
				{
					if(isMajLabel[j])
						majBags.get(j).add(i);
				}
		...
		while(deleteInsNum>0){
			...
			if(isAllMinority){
				break;  //all labels is majority (<= MeanIR)
				//recalculate MeanIR, IRLbl, c1[] and mc
			}
	        ...
	        for(int i=0; i<labelNum; i++){
	        	...
	        	//recalculate IRLbl of labelindices[i]
	        	double newIRLbl= mc*1.0/c1[i]; 
	            if(newIRLbl>=MeanIR){
	            	isMajLabel[i]=false;  //tag label j to the minority label
	            	continue;
	            }
