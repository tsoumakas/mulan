...
public class MLHNLabelCorrelation extends MultiLabelLearnerBase{
	...
	private int EachNum=5;
	private int learnNumRP=10;
	private int learnNumGD=10;
	...
	private MultiLabelHyperNetWorkType type=MultiLabelHyperNetWorkType.MLHN_GC;
	...
	//record the index of hyperedge than is matched with processed train data
	...
	private void buildProcessTrainDataset(MultiLabelInstances train) throws InvalidDataException, Exception{
		baseLeaner.build(train);
		 ...
		 processTrainDataset=new Instances("TrainDataSetProcessedBy"+BaseFuction.Get_Classifier_name(baseLeaner), 
	...
	private void initialize() throws Exception{
 	...
 	private void initializeGC() throws Exception{
		 for(int i=0;i<processTrainDataset.numInstances();i++){
			 ...
			 int c=0;
			 while(c<EachNum){
				 Integer vertexArray[]=BaseFuction.randomIntegerArray(numLabels-1,0,order);
	...
	private void initializeLC() throws Exception{
		 for(int i=0;i<processTrainDataset.numInstances();i++){
			 ...
			 int c=0;
			 while(c<EachNum){
				 Integer vertexArray[]=BaseFuction.randomIntegerArray(2*numLabels-1,numLabels,order);
	...
	private void initializeGLC() throws Exception{
		 for(int i=0;i<processTrainDataset.numInstances();i++){
			 ...
			 while(c<EachNum){
				 ...
				 Boolean valueTypeArray[]=new Boolean[order];
				 for(int j=0;j<order;j++){
					 valueTypeArray[j]=(BaseFuction.randomInt(0, 1)==0);
					 if(!valueTypeArray[j]){
	...
	private void caculatePreThreld(){
	}
	
	private void caculatePreThreldInternal1(){
		for(int j=0;j<numLabels;j++){
			int t=processTrainDataset.numInstances();
			int t1=0,t0=0,c=0,c1=0,c0=0;
